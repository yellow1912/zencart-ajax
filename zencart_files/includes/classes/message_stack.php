...
// begin editing for ajax
  ...
  function _add($class, $message, $type = 'error') {
	...
	function add($class, $message, $type = 'error') {
		global $Ajax;
		if(is_object($Ajax))
			$Ajax->addMessage($class, $message, $type);
		else
			$this->_add($class, $message, $type);
	}
	
  function _add_session($class, $message, $type = 'error') {
  ...
  function add_session($class, $message, $type = 'error') {
		global $Ajax;
		if(is_object($Ajax))
			$Ajax->addSessionMessage($class, $message, $type);
		else
			$this->_add_session($class, $message, $type);
	}
// end editing for ajax	
  function reset() {
    $this->messages = array();
  }
